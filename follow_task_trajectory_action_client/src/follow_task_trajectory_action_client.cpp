 ...
 *
 * Title:   follow_task_trajectory_action_client.cpp
...
public:
  explicit FollowTaskTrajectoryActionClient(const rclcpp::NodeOptions& options) : Node("follow_task_trajectory_action_client", options)
  {
    ...
    std::filesystem::path package_share_directory = ament_index_cpp::get_package_share_directory("follow_task_trajectory_action_client");
    ...
    while (reader.has_next())
    {
      ...
      if (msg->topic_name != "/task_space_trajectory")
      {
        continue;
      }
    ...
    {
      RCLCPP_ERROR(this->get_logger(), "No valid task space trajectory found in input bag file: %s", input_path.c_str());
      rclcpp::shutdown();
      return;
    }
  ...
  bool send_first_trajectory_point()
  {
    ...
    std::shared_future<FollowTaskTrajectoryGoalHandle::SharedPtr> future_goal_handle = client_ptr_->async_send_goal(goal_msg);
    ...
    FollowTaskTrajectoryGoalHandle::SharedPtr goal_handle = future_goal_handle.get();
    if (!goal_handle)
    {
    ...
    std::shared_future<FollowTaskTrajectoryGoalHandle::WrappedResult> result_future = client_ptr_->async_get_result(goal_handle);
  ...
  void send_trajectory()
  {
    ...
    send_goal_options.result_callback = std::bind(&FollowTaskTrajectoryActionClient::result_callback, this, std::placeholders::_1);
...
private:
  ...
  void goal_response_callback(const FollowTaskTrajectoryGoalHandle::SharedPtr& goal_handle)
  {
    if (!goal_handle)
    {
      RCLCPP_ERROR(this->get_logger(), "Goal was rejected by server");
  ...
  void result_callback(const FollowTaskTrajectoryGoalHandle::WrappedResult& result)
  {
    switch (result.code)
    {
      case rclcpp_action::ResultCode::SUCCEEDED:
      {
        ...
        writer.open((output_path_ / ("follow_task_space_trajectory_result_" + current_time)).string());
...
};  // class FollowTaskTrajectoryActionClient

}  // namespace follow_task_trajectory_action_client_cpp

RCLCPP_COMPONENTS_REGISTER_NODE(follow_task_trajectory_action_client_cpp::FollowTaskTrajectoryActionClient)
