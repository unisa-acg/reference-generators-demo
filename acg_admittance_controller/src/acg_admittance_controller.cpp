...
controller_interface::CallbackReturn AdmittanceController::on_init()
{
  ...
  try
  {
    admittance_controller_param_handler_ = std::make_shared<acg_admittance_controller::ParamListener>(get_node());
  }
  ...
  state_publisher_ = std::make_shared<acg_diagnostics::PeriodicPublisher<acg_control_msgs::msg::AdmittanceControllerState>>(
...
bool AdmittanceController::compute_control_law(const rclcpp::Time& time, const rclcpp::Duration& period)
{
  ...
  acg_kinematics::transform_wrench_frame(*kinematics_, joint_space_state_.positions, motion_to_desired_frame_transform_msg, motion_reference_frame_,
...
acg_control_msgs::msg::TaskSpacePoint AdmittanceController::apply_reference_scaling_()
{
