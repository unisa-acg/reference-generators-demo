...
protected:
  ...
  std::unique_ptr<xxx_semantic_components::AccelCommand> accel_command_;
};
...
void ConfiguredAccelCommandTest::SetUp()
{
  ...
  exp_size_ = 6;
  accel_command_ = std::make_unique<xxx_semantic_components::AccelCommand>(device_name_);
...
TEST_F(AccelCommandTest, test_wrong_size_constructor)
{
  ...
  EXPECT_THROW(xxx_semantic_components::AccelCommand(device_name_, std::vector<std::string>{ "wrong_size" }), std::invalid_argument);
}
...
TEST_F(AccelCommandTest, test_custom_interface_names)
{
  ...
  accel_command_ = std::make_unique<xxx_semantic_components::AccelCommand>(device_name_, custom_interface_names);
