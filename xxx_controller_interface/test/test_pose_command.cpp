...
protected:
  ...
  std::unique_ptr<xxx_semantic_components::PoseCommand> pose_command_;
};
...
void ConfiguredPoseCommandTest::SetUp()
{
  ...
  exp_size_ = 7;
  pose_command_ = std::make_unique<xxx_semantic_components::PoseCommand>(device_name_);
...
TEST_F(PoseCommandTest, test_wrong_size_constructor)
{
  ...
  EXPECT_THROW(xxx_semantic_components::PoseCommand(device_name_, std::vector<std::string>{ "wrong_size" }), std::invalid_argument);
}
...
TEST_F(PoseCommandTest, test_custom_interface_names)
{
  ...
  pose_command_ = std::make_unique<xxx_semantic_components::PoseCommand>(device_name_, custom_interface_names);
