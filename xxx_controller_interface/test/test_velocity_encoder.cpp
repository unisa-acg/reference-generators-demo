...
void ConfiguredVelocityEncoderTest::SetUp()
{
  ...
  exp_size_ = 7;
  velocity_encoder_ = std::make_unique<xxx_semantic_components::VelocityEncoder>(sensor_name_, exp_size_);
...
TEST_F(VelocityEncoderTest, test_wrong_size_constructor)
{
  ...
  EXPECT_THROW(std::make_unique<xxx_semantic_components::VelocityEncoder>(sensor_name_, 3, std::vector<std::string>{ "wrong_size" }),
...
TEST_F(VelocityEncoderTest, test_custom_interface_names)
{
  ...
  velocity_encoder_ = std::make_unique<xxx_semantic_components::VelocityEncoder>(sensor_name_, custom_interface_names.size(), custom_interface_names);
