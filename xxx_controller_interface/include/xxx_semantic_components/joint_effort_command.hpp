 ...
 *
 * Title:   joint_effort_command.hpp
 * Author:  Davide Risi
 * Org.:    UNISA
 * Date:    Feb 17, 2025
 *
 * This class implements a joint effort command semantic component.
 *
 * -------------------------------------------------------------------
 */
...
class JointEffortCommand : public semantic_components::SemanticComponentCommandInterface<std::vector<double>>
{
private:
  std::vector<std::string> generate_interface_names_(const std::string& name, const std::size_t size, const std::vector<std::string>& interface_names)
  {
    if (!interface_names.empty())
    {
      ...
      {
        throw std::invalid_argument("JointEffortCommand: interface_names size (" + std::to_string(interface_names.size()) + ") must be equal to " +
                                    std::to_string(size));
      }
    ...
    else
    {
      ...
      {
        interface_names.emplace_back(name + "/joint" + std::to_string(i) + "/effort");
      }
      return interface_names;
    }
  }
...
public:
  JointEffortCommand(const std::string& name, const std::size_t size = 0, const std::vector<std::string>& interface_names = {})
    : SemanticComponentCommandInterface(name, generate_interface_names_(name, size, interface_names))
  {}

  ~JointEffortCommand() = default;
