...
public:
  ...
  std::unique_ptr<xxx_kinematics::RTKinematicsSolver> robot_kinematics;
  std::string tip_link;
...
TEST_F(KinematicsTest, TestComputeForwardKinematics)
{
  ...
  robot_kinematics = std::make_unique<xxx_kinematics::RTKinematicsSolver>();
...
TEST_F(KinematicsTest, TestComputeFrameToFrameTransform)
{
  ...
  xxx_kinematics::compute_frame_to_frame_transform(*kinematics_interface, joint_positions, start_frame, end_frame, transform);
...
TEST_F(KinematicsTest, TestChangeWrenchFrame)
{
  ...
  xxx_kinematics::transform_wrench_frame(*kinematics_interface, joint_positions, desired_wrench_frame, wrench_frame, wrench);
...
TEST_F(KinematicsTest, TestChangeWrenchMessageFrame)
{
  ...
  // Change the wrench frame
  xxx_kinematics::transform_wrench_frame(*kinematics_interface, joint_positions, desired_wrench_frame, wrench);
...
TEST_F(KinematicsTest, TestChangeWrenchFrameFromTransform)
{
  ...
  // Change the wrench frame
  xxx_kinematics::transform_wrench_frame(*kinematics_interface, joint_positions, desired_frame_transform, wrench);
...
TEST_F(KinematicsTest, TestChangeWrenchFrameFromTransformMessage)
{
  ...
  // Change the wrench frame
  xxx_kinematics::transform_wrench_frame(*kinematics_interface, joint_positions, desired_frame_transform, "link2", wrench_frame, wrench);
...
TEST_F(KinematicsTest, TestSetTaskSpacePointFrame)
{
  ...
  xxx_control_msgs::msg::TaskSpacePoint expected_point;
  ...
  xxx_kinematics::transform_task_space_point_frames(*kinematics_interface, joint_positions, desired_motion_frame, desired_wrench_frame,
                                                    task_space_point);
...
TEST_P(ComputePoseErrorTestSuite, ComputePoseError)
{
  ...
  xxx_kinematics::compute_pose_error(param.desired_pose, param.current_pose, error);
...
TEST_P(ComputeTwistErrorTestSuite, ComputeTwistError)
{
  ...
  Eigen::Matrix<double, 6, 1> error;
  xxx_kinematics::compute_twist_error(param.desired_twist, param.current_twist, error);
  for (Eigen::Index i = 0; i < param.expected_error.size(); ++i)
  {
    EXPECT_NEAR(error(i), param.expected_error(i), ABS_EPSILON) << "Error at index " << i << ": " << error(i) << " != " << param.expected_error(i);
  }
...
TEST_P(ComputeWrenchErrorTestSuite, ComputeWrenchError)
{
  ...
  Eigen::Matrix<double, 6, 1> error;
  xxx_kinematics::compute_wrench_error(param.desired_wrench, param.current_wrench, error);
  for (Eigen::Index i = 0; i < param.expected_error.size(); ++i)
  {
    EXPECT_NEAR(error(i), param.expected_error(i), ABS_EPSILON) << "Error at index " << i << ": " << error(i) << " != " << param.expected_error(i);
  }
...
TEST_P(ComputeEuclideanDistanceTestSuite, ComputeEuclideanDistance)
{
  ...
  double distance = xxx_kinematics::compute_euclidean_distance(param.point1, param.point2);
...
TEST_P(ComputeAngularDistanceTestSuite, ComputeAngularDistance)
{
  ...
  double distance = xxx_kinematics::compute_angular_distance(param.quat1, param.quat2);
  EXPECT_NEAR(distance, param.expected_distance, ABS_EPSILON);
}
...
TEST_P(IsPoseCloseTestSuite, IsPoseClose)
{
  ...
  bool result = xxx_kinematics::is_pose_close(param.pose1, param.pose2, param.position_tolerance, param.orientation_tolerance);
